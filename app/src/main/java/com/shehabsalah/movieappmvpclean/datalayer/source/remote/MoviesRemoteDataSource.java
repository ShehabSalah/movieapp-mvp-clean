...
public class MoviesRemoteDataSource implements MoviesDataSource {
    ...
    public void saveTopRatedMovies(ArrayList<Movie> movies) {
        // Not required for the remote data source because the {@link MoviesLocalDataSource} handles
        // the saving implementation of the top rated movies.
    }

    @Override
    public void saveMostPopularMovies(ArrayList<Movie> movies) {
        // Not required for the remote data source because the {@link MoviesLocalDataSource} handles
        // the saving implementation of the most popular movies.
    }

    @Override
    public void saveMovieTrailers(ArrayList<MovieTrailers> movieTrailers, int movieId) {
        // Not required for the remote data source because the {@link MoviesLocalDataSource} handles
        // the saving implementation of the movie trailers.
    }

    @Override
    public void saveMovieReviews(ArrayList<MovieReviews> movieReviews, int movieId) {
        // Not required for the remote data source because the {@link MoviesLocalDataSource} handles
        // the saving implementation of the movie reviews.
    }

    @Override
    public void updateMovie(Movie movie) {
        // Not required for the remote data source because the updating of the movie done only
        // on the local data source and {@link MoviesLocalDataSource} handles that.
    }

    @Override
    public void deleteAllMovies() {
        // Not required for the remote data source because the {@link MoviesLocalDataSource} handles
        // the deleting implementation of the movies.
    }

    @Override
    public void deleteMovieTrailers(int movieId) {
        // Not required for the remote data source because the {@link MoviesLocalDataSource} handles
        // the deleting implementation of the movie trailers.
    }

    @Override
    public void deleteMovieReviews(int movieId) {
        // Not required for the remote data source because the {@link MoviesLocalDataSource} handles
        // the deleting implementation of the movie reviews.
    }
}
